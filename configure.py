...
def parse_arguments():
    ...
    parser.add_argument('--project-name', required=True, help="The new project name")
...
def modify_file(base_path, library_name, file_subpath, replacements, new_file_name=None):
    """
...
def rename_project(base_path, library_name):
    ...
    modify_file(base_path, library_name, 'cmake/my_libraryConfig.cmake.in', {
        '@PACKAGE_INIT@': '@PACKAGE_INIT@',
        'my_libraryTargets.cmake': f'{library_name}Targets.cmake'
    }, f'{library_name}Config.cmake.in')

    modify_file(base_path, library_name, 'include/my_library/my_library_export.h', {
        'MY_LIBRARY': library_name.upper()
    }, f'{library_name}_export.h')

    modify_file(base_path, library_name, 'include/my_library/my_library.h', {
        'MY_LIBRARY': library_name.upper(),
    ...
    modify_file(base_path, library_name, 'my_library/internal/my_library_internal.h', {}, f'{library_name}_internal.h')

    modify_file(base_path, library_name, 'my_library/source/my_library_internal.cpp', {
        'my_library_internal.h': f'{library_name}_internal.h',
        'MY_LIBRARY_VERSION': f'{library_name.upper()}_VERSION',
        'BUILDING_MY_LIBRARY': f'BUILDING_{library_name.upper()}'
    }, f'{library_name}_internal.cpp')

    modify_file(base_path, library_name, 'my_library/source/my_library.cpp', {
        'my_library/my_library.h': f'{library_name}/{library_name}.h',
        'my_library_internal.h': f'{library_name}_internal.h'
    }, f'{library_name}.cpp')

    modify_file(base_path, library_name, 'my_library/CMakeLists.txt', {
    ...
    modify_file(base_path, library_name, 'tests/CMakeLists.txt', {
        'my_library': library_name
    })

    modify_file(base_path, library_name, 'CMakeLists.txt', {
        'project(lib_my_library': f'project(lib_{library_name}',
        'add_subdirectory(my_library)': f'add_subdirectory({library_name})'
    })

    modify_file(base_path, library_name, 'tests/example_01.cpp', {
        '#include "my_library/my_library.h"': f'#include "{library_name}/{library_name}.h"'
    })
